...
def fit(sub_df, model_name, stakes, sophistication, final):
    ...
    fit = scipy.optimize.minimize(
        model_function,
        params,
        args=([sub_df, stakes, final, kappa_equivalent]),
        method="L-BFGS-B",
        bounds=bounds,
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Model fitting code for the two stage task using MLE or MAP"
    )
    parser.add_argument(
        "-s", "--sub_path", required=True, help="Source file for subject csv"
    )
    parser.add_argument(
        "-m",
        "--model_name",
        required=True,
        help="specify the model type you would like to run",
    )
    parser.add_argument(
        "-n",
        "--num_inits",
    ...
    parser.add_argument(
        "-x", "--stakes", required=True, help="how many w parameters to fit [1,2,4]"
    )
    parser.add_argument(
        "-c",
        "--num_cores",
    ...
    model_name = (
        args.model_name
    )  # Which model you would like to fit ('current options')
    ...
    if sophistication:
        filename = os.path.join(
            output_path,
            f"{sub}_{model_name}_w{stakes}_{sophistication}_transmats_fits.pickle",
        )
    else:
        filename = os.path.join(
            output_path, f"{sub}_{model_name}_w{stakes}_final_transmats_fits.pickle"
        )
