...
def apply_priors(params, model_func_params):
	...
	"""
    LL = MB_MF_rllik_learn_mat_arms(params, model_func_params[0], model_func_params[1], model_func_params[2],model_func_params[3])
...
def MB_MF_rllik_learn_mat_arms(params, sub_df, stakes=4, final=True, kappa_equivalent=False):
    """
    ...
    lr = params[1]
    lamb = params[2]
    w_lo_low_arm = params[3]  # if stakes == 2 this will be w_lo/ if stakes == 2c this will be w_low_arm
    ...
    for trial in range(0, N):
        ...
        Q = w * Qmb + (1 - w) * np.atleast_2d(Qmf[s1 - 1, :]).T + st * np.atleast_2d(
            M[s1 - 1, :]).T + respst * R  # have to do some weird things to change row vecs to column vecs
