...
def model_mat_fits_points_corr(model_mat_fits, grouped_stakes):
    ...
    melted_mmp = model_mat_points[
        ['subid', 'Visual cooccurrence', 'Direct item association', 'Indirect item association',
         'Points earned in decision-making task', 'rt_2']].melt(
        id_vars=['subid', 'Points earned in decision-making task', 'rt_2'], var_name='Grouping Model',
    ...
    print('Correlation of Visual Co-occurence β with Points earned in decision-making task')
    print(pg.corr(
        melted_mmp[melted_mmp['Grouping Model'] == 'Visual cooccurrence']['Points earned in decision-making task'],
        melted_mmp[melted_mmp['Grouping Model'] == 'Visual cooccurrence']['model_matrix_fit']))
    print('Correlation of Direct reward association β with Points earned in decision-making task')
    print(pg.corr(
        melted_mmp[melted_mmp['Grouping Model'] == 'Direct item association']['Points earned in decision-making task'],
        melted_mmp[melted_mmp['Grouping Model'] == 'Direct item association']['model_matrix_fit']))
    print('')
    print('Correlation of Indirect outcome-linked association β with Points earned in decision-making task')
    print(pg.corr(melted_mmp[melted_mmp['Grouping Model'] == 'Indirect item association'][
...
def model_mat_fits_w_corr(model_mat_fits, w1_map_df):
    ...
    mm1w_melt = model_mat_1w_map.melt(id_vars=['subid', 'high_arm', 'w fit'], var_name='Grouping Model',
                                      value_name=r'Model Matrix Fit ($\bar{\beta}$)')
    ...
    print(pg.corr(mm1w_melt[mm1w_melt['Grouping Model'] == 'Visual cooccurrence']['w fit'],
                  mm1w_melt[mm1w_melt['Grouping Model'] == 'Visual cooccurrence']['model_matrix_fit']))

    print('')
    print('Correlation of Direct reward association β with w fit')
    print(pg.corr(mm1w_melt[mm1w_melt['Grouping Model'] == 'Direct item association']['w fit'],
                  mm1w_melt[mm1w_melt['Grouping Model'] == 'Direct item association']['model_matrix_fit']))

    print('')
    print('Correlation of Indirect outcome-linked association β with w fit')
    print(pg.corr(mm1w_melt[mm1w_melt['Grouping Model'] == 'Indirect item association']['w fit'],
...
def memory_dprime_ttest(dprime_df):
    ...
    print(pg.ttest(dprime_stat_df['dprime_lbst_high'], dprime_stat_df['dprime_lbst_low'], paired=True))
    print('')
    print('paired sample t-test between dprime lure for high and low arm')
    print(pg.ttest(dprime_stat_df['dprime_lt_high'], dprime_stat_df['dprime_lt_low'], paired=True))
    print('')


def model_based_condition_anova(w4_map_df):
    melted_4w_map = w4_map_df[
        ['subid', 'w low stakes low arm fit', 'w high stakes low arm fit', 'w low stakes high arm fit',
         'w high stakes high arm fit']].melt(id_vars=['subid'], var_name='type of', value_name='w')
    melted_4w_map['arm'] = 'high'
    melted_4w_map.loc[np.where(melted_4w_map['type of'] == 'w low stakes low arm fit')[0], 'arm'] = 'low'
    melted_4w_map.loc[np.where(melted_4w_map['type of'] == 'w high stakes low arm fit')[0], 'arm'] = 'low'

    melted_4w_map['stakes'] = '5'
    melted_4w_map.loc[np.where(melted_4w_map['type of'] == 'w low stakes low arm fit')[0], 'stakes'] = '1'
    melted_4w_map.loc[np.where(melted_4w_map['type of'] == 'w low stakes high arm fit')[0], 'stakes'] = '1'
    aov = pg.rm_anova(dv='w', within=['arm', 'stakes'], subject='subid', data=melted_4w_map,detailed=True)
    # Pretty printing of ANOVA summary
    pg.print_table(aov)
    print(f"5x w parameter is {melted_4w_map.loc[np.where(melted_4w_map['stakes'] == '5'), 'w'].mean()}")
    print(f"1x w parameter is {melted_4w_map.loc[np.where(melted_4w_map['stakes'] == '1'), 'w'].mean()}")



def model_fitting_descriptive_stats(w1_map_df):
    print('Summary stats for different components of model')
    print(w1_map_df[['beta fit', 'alpha fit', 'lambda fit', 'w low stakes low arm fit', 'stickiness fit',
                     'resp stickiness fit', 'eta fit', 'kappa fit', 'LL'
                     ]].describe())
    print('')


def main():
    parser = argparse.ArgumentParser(description='Code to generate primary stats for behavioral paper')
