...
def w1_points_rt_corr(w1_map_df, grouped_stakes):
    ...
    pcor = pg.pairwise_corr(perfvsw, columns=['w fit', 'Points earned in decision-making task',
                                              'rt_1'], method='pearson')
...
def dprime_corr(w1_map_df, dprime_df):
    ...
    pcor = pg.pairwise_corr(dprime_mb, columns=['w fit', 'dprime_lbst_high', 'dprime_lbst_low',
                                                'dprime_lt_high', 'dprime_lt_low'], method='pearson')
    print(pcor.round(4).iloc[
          0:4])  # only printing correlations with w fit because we don't care about dprime correlations with each other


def model_mat_fits_corr(model_mat_fits):
    pcor = pg.pairwise_corr(model_mat_fits, columns=['Visual cooccurrence', 'Direct item association',
                                                     'Indirect item association'], method='pearson')
    print(pcor.round(4))  #


def main():
    parser = argparse.ArgumentParser(description='Code to generate supplementary stats for behavioral paper')
