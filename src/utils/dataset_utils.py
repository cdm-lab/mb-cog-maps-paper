...
def make_good_subjects_list(input_path, output_path, exp_num=2):
    ...
    for sub in subids:  # create bad_subjects list based on RT threshold
        ...
        if exp_num == 2:
            percentages_mem = (
                back_df[back_df["subid"] == sub]["rt"].value_counts(normalize=True)
                * 100
            )
...
def model_fit_threshold(input_path, output_path, subjects, exp_num):
    ...
    for sub in subids:
        ...
        if (
            sub_w1_map_df["LL"].values[0] > 172.846
        ):  # this number is from generating 1000 randomly selecting agents
            bad_subjects.append(sub)
...
def make_memory_df(input_path, output_path, subjects=None, exp_num=2):
    ...
    subids = subjects
    assert (
        len(set(subids) - set(back_df.subid.unique())) <= 0
    ), "trying to grab participants that don't have memory data!"
    ...
    if exp_num == 2:
        ...
        olds_correct = np.where(
            ((clean_back_df["response"] == 0.0) | (clean_back_df["response"] == 1.0))
            & (clean_back_df["lure_or_no"] == "target")
        )[0]
        news_correct = np.where(
            ((clean_back_df["response"] == 2.0) | (clean_back_df["response"] == 3.0))
            & (clean_back_df["lure_or_no"] != "target")
        )[0]
        clean_back_df.loc[olds_correct, "accuracy"] = 1
        clean_back_df.loc[news_correct, "accuracy"] = 1
        clean_back_df["background_stim"] = clean_back_df["backgroundImg"].apply(
            lambda x: os.path.basename(x)[:-4]
        )
        clean_back_df["left_stim"] = clean_back_df["left_obj"].apply(
            lambda x: os.path.basename(x)[:-4]
        )
        clean_back_df["right_stim"] = clean_back_df["right_obj"].apply(
            lambda x: os.path.basename(x)[:-4]
        )
        clean_back_df["inside_or_outside"] = "outside"
        inside_indices = np.where(
            clean_back_df["backgroundImg"].str.extract("(forest|desert)").isna()
        )[0]
        clean_back_df.loc[inside_indices, "inside_or_outside"] = "inside"
        clean_back_df["subject_high_arm"] = clean_back_df["subid"].apply(
            lambda x: sub_dict[x]["high_arm"]
        )
        clean_back_df["is_high_arm"] = (
            clean_back_df["inside_or_outside"] == clean_back_df["subject_high_arm"]
        )

        mem_df = (
            clean_back_df.groupby(["subid", "lure_or_no", "is_high_arm"])["accuracy"]
            .mean()
            .reset_index()
        )
        ...
        target_adjust = {
            0: 0.5 / num_target_trials,
            1: ((num_target_trials - 0.5) / num_target_trials),
        }
        mismatch_adjust = {
            0: 0.5 / num_mismatch_trials,
            1: ((num_mismatch_trials - 0.5) / num_mismatch_trials),
        }
        ...
        mem_df.loc[mismatch_indices, "accuracy"] = mem_df.loc[
            mismatch_indices, "accuracy"
        ].replace(mismatch_adjust)
        mem_df.loc[non_mismatch_indices, "accuracy"] = mem_df.loc[
            non_mismatch_indices, "accuracy"
        ].replace(target_adjust)
        for sub in mem_df["subid"].unique():
            sub_df = mem_df[mem_df["subid"] == sub].reset_index()
            high_target_mask = np.where(
                (sub_df["is_high_arm"] == True) & (sub_df["lure_or_no"] == "target")
            )[0]
            low_target_mask = np.where(
                (sub_df["is_high_arm"] == False) & (sub_df["lure_or_no"] == "target")
            )[0]
            high_lure_mask = np.where(
                (sub_df["is_high_arm"] == True) & (sub_df["lure_or_no"] == "lure")
            )[0]
            low_lure_mask = np.where(
                (sub_df["is_high_arm"] == False) & (sub_df["lure_or_no"] == "lure")
            )[0]
            high_mismatch_mask = np.where(
                (sub_df["is_high_arm"] == True)
                & (sub_df["lure_or_no"] == "lure_but_seen")
            )[0]
            low_mismatch_mask = np.where(
                (sub_df["is_high_arm"] == False)
                & (sub_df["lure_or_no"] == "lure_but_seen")
            )[0]
